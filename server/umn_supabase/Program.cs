...
public class Program
{
    ...
    public static async Task getReplaysAndProcess()
    {
        ...
        foreach (var replay in replays)
        {
            ...
            // Download replay
            var replayBytes = await supabase.Storage.From("Replays").Download("queue/" + replayName,
                (bytes, progress) => { Console.WriteLine($"Downloaded {progress}%"); });
    ...
    public static async Task<bool> processMatch(string fileName)
    {
        ...
        var replay =
            await ReplayReader.ReadReplayAsync(file.FullName, new ReplayReaderOptions(), CancellationToken.None);


        Console.WriteLine("Processing match: " + fileName);

        Metadata2 metadata = ((ROFL2)replay.Result).Metadata;
        ...
        PlayerStats2[] playerStats = metadata.PlayerStatistics;

        Player[] players = new Player[playerStats.Length];
        MatchStats[] matchStats = new MatchStats[playerStats.Length];
        

        // Swap where I am getting the match_id from
        // think I will go with the format of team1_name vs team2_name - week - season
        var teams = await GetTeams(metadata);
        Match match = GetMatch(metadata, file.Name);
        ...
        for (int i = 0; i < playerStats.Length; i++)
        {
            ...
            MatchStats matchStat = GetPlayerMatchStats(playerStats[i], match.match_id);
            //
    ...
    public static async Task<(Guid, Guid, Guid)> GetTeams(Metadata2 metadata)
    {
        PlayerStats2[] playerStats = metadata.PlayerStatistics;
        ...
        for (int i = 0; i < playerStats.Length; i++)
        {
            ...
            // Make a call to the Riot API to get the player's account information from the puuid
    ...
    public static async Task saveUsers()
    {
        ...
        foreach (var file in files)
        {
            ...
            var replay = await ReplayReader.ReadReplayAsync(fileInfo.FullName, new ReplayReaderOptions(), CancellationToken.None);
            var metadata = ((ROFL2)replay.Result).Metadata;
    ...
    public static Match GetMatch(Metadata2 metadata, String match_id)
    {
        // Get the week from the user
        ...
        Match match = new Match
        {
            ...
            winner_team_id = null,
            is_playoffs = false,
            week = int.TryParse(week, out int week_int) ? week_int : 0
        };
    ...
    public static MatchStats GetPlayerMatchStats(PlayerStats2 playerStats, String match_id)
    {
        MatchStats matchStats = new MatchStats
        {
            ...
            barracks_takedowns = int.TryParse(playerStats.BarracksTakedowns, out int barracks_takedowns)
                ? barracks_takedowns
                : 0,
            bounty_level = int.TryParse(playerStats.BountyLevel, out int bounty_level) ? bounty_level : 0,
            champions_killed = int.TryParse(playerStats.ChampionsKilled, out int champions_killed)
                ? champions_killed
                : 0,
            champion_mission_stat0 = int.TryParse(playerStats.ChampionMissionStat0, out int champion_mission_stat0)
                ? champion_mission_stat0
                : 0,
            champion_mission_stat1 = int.TryParse(playerStats.ChampionMissionStat1, out int champion_mission_stat1)
                ? champion_mission_stat1
                : 0,
            champion_mission_stat2 = int.TryParse(playerStats.ChampionMissionStat2, out int champion_mission_stat2)
                ? champion_mission_stat2
                : 0,
            champion_mission_stat3 = int.TryParse(playerStats.ChampionMissionStat3, out int champion_mission_stat3)
                ? champion_mission_stat3
                : 0,
            champion_transform = int.TryParse(playerStats.ChampionTransform, out int champion_transform)
                ? champion_transform
                : 0,
            consumable_purchased = int.TryParse(playerStats.ConsumablesPurchased, out int consumable_purchased)
            ...
            friendly_dampen_lost = int.TryParse(playerStats.FriendlyDampenLost, out int friendly_dampen_lost)
                ? friendly_dampen_lost
                : 0,
            friendly_hq_lost =
                int.TryParse(playerStats.FriendlyHQLost, out int friendly_hq_lost) ? friendly_hq_lost : 0,
            friendly_turret_lost = int.TryParse(playerStats.FriendlyTurretLost, out int friendly_turret_lost)
                ? friendly_turret_lost
                : 0,
            game_ended_in_early_surrender =
                bool.TryParse(playerStats.GameEndedInEarlySurrender, out bool game_ended_in_early_surrender)
                    ? game_ended_in_early_surrender
                    : false,
            game_ended_in_surrender = bool.TryParse(playerStats.GameEndedInSurrender, out bool game_ended_in_surrender)
            ...
            largest_critical_strike = int.TryParse(playerStats.LargestCriticalStrike, out int largest_critical_strike)
                ? largest_critical_strike
                : 0,
            largest_killing_spree = int.TryParse(playerStats.LargestKillingSpree, out int largest_killing_spree)
                ? largest_killing_spree
                : 0,
            largest_multi_kill = int.TryParse(playerStats.LargestMultiKill, out int largest_multi_kill)
                ? largest_multi_kill
                : 0,
            level = int.TryParse(playerStats.Level, out int level) ? level : 0,
            longest_time_spent_living =
                int.TryParse(playerStats.LongestTimeSpentLiving, out int longest_time_spent_living)
                    ? longest_time_spent_living
                    : 0,
            magic_damage_dealt_player =
                int.TryParse(playerStats.MagicDamageDealtPlayer, out int magic_damage_dealt_player)
                    ? magic_damage_dealt_player
                    : 0,
            magic_damage_dealt_to_champions =
                int.TryParse(playerStats.MagicDamageDealtToChampions, out int magic_damage_dealt_to_champions)
                    ? magic_damage_dealt_to_champions
                    : 0,
            magic_damage_taken = int.TryParse(playerStats.MagicDamageTaken, out int magic_damage_taken)
                ? magic_damage_taken
                : 0,
            ...
            neutral_minions_killed = int.TryParse(playerStats.NeutralMinionsKilled, out int neutral_minions_killed)
                ? neutral_minions_killed
                : 0,
            neutral_minions_killed_enemy_jungle =
                int.TryParse(playerStats.NeutralMinionsKilledEnemyJungle, out int neutral_minions_killed_enemy_jungle)
                    ? neutral_minions_killed_enemy_jungle
                    : 0,
            neutral_minions_killed_your_jungle =
                int.TryParse(playerStats.NeutralMinionsKilledYourJungle, out int neutral_minions_killed_your_jungle)
                    ? neutral_minions_killed_your_jungle
                    : 0,
            node_capture = int.TryParse(playerStats.NodeCapture, out int node_capture) ? node_capture : 0,
            node_capture_assist = int.TryParse(playerStats.NodeCaptureAssist, out int node_capture_assist)
                ? node_capture_assist
                : 0,
            node_neutralize = int.TryParse(playerStats.NodeNeutralize, out int node_neutralize) ? node_neutralize : 0,
            node_neutralize_assist = int.TryParse(playerStats.NodeNeutralizeAssist, out int node_neutralize_assist)
                ? node_neutralize_assist
                : 0,
            num_deaths = int.TryParse(playerStats.NumDeaths, out int num_deaths) ? num_deaths : 0,
            objectives_stolen = int.TryParse(playerStats.ObjectivesStolen, out int objectives_stolen)
                ? objectives_stolen
                : 0,
            objectives_stolen_assists =
                int.TryParse(playerStats.ObjectivesStolenAssists, out int objectives_stolen_assists)
                    ? objectives_stolen_assists
                    : 0,
            ...
            perk_primary_style = int.TryParse(playerStats.PerkPrimaryStyle, out int perk_primary_style)
                ? perk_primary_style
                : 0,
            perk_sub_style = int.TryParse(playerStats.PerkSubStyle, out int perk_sub_style) ? perk_sub_style : 0,
            physical_damage_dealt_player =
                int.TryParse(playerStats.PhysicalDamageDealtPlayer, out int physical_damage_dealt_player)
                    ? physical_damage_dealt_player
                    : 0,
            physical_damage_dealt_to_champions =
                int.TryParse(playerStats.PhysicalDamageDealtToChampions, out int physical_damage_dealt_to_champions)
                    ? physical_damage_dealt_to_champions
                    : 0,
            physical_damage_taken = int.TryParse(playerStats.PhysicalDamageTaken, out int physical_damage_taken)
            ...
            players_that_muted_me = int.TryParse(playerStats.PlayersThatMutedMe, out int players_that_muted_me)
            ...
            sight_wards_bought_in_game =
                int.TryParse(playerStats.SightWardsBoughtInGame, out int sight_wards_bought_in_game)
                    ? sight_wards_bought_in_game
                    : 0,
            ...
            summon_spell1_cast = int.TryParse(playerStats.SummonSpell1Cast, out int summon_spell1_cast)
                ? summon_spell1_cast
                : 0,
            summon_spell2_cast = int.TryParse(playerStats.SummonSpell2Cast, out int summon_spell2_cast)
                ? summon_spell2_cast
                : 0,
            team = int.TryParse(playerStats.Team, out int team) ? team : 0,
            team_early_surrendered = bool.TryParse(playerStats.TeamEarlySurrendered, out bool team_early_surrendered)
            ...
            time_ccing_others = int.TryParse(playerStats.TimeCCingOthers, out int time_ccing_others)
                ? time_ccing_others
                : 0,
            time_of_from_last_disconnect =
                int.TryParse(playerStats.TimeOfFromLastDisconnect, out int time_of_from_last_disconnect)
                    ? time_of_from_last_disconnect
                    : 0,
            time_played = int.TryParse(playerStats.TimePlayed, out int time_played) ? time_played : 0,
            time_spent_disconnected = int.TryParse(playerStats.TimeSpentDisconnected, out int time_spent_disconnected)
                ? time_spent_disconnected
                : 0,
            total_damage_dealt = int.TryParse(playerStats.TotalDamageDealt, out int total_damage_dealt)
                ? total_damage_dealt
                : 0,
            total_damage_dealt_to_buildings =
                int.TryParse(playerStats.TotalDamageDealtToBuildings, out int total_damage_dealt_to_buildings)
                    ? total_damage_dealt_to_buildings
                    : 0,
            total_damage_dealt_to_champions =
                int.TryParse(playerStats.TotalDamageDealtToChampions, out int total_damage_dealt_to_champions)
                    ? total_damage_dealt_to_champions
                    : 0,
            total_damage_dealt_to_objectives =
                int.TryParse(playerStats.TotalDamageDealtToObjectives, out int total_damage_dealt_to_objectives)
                    ? total_damage_dealt_to_objectives
                    : 0,
            total_damage_dealt_to_turrets =
                int.TryParse(playerStats.TotalDamageDealtToTurrets, out int total_damage_dealt_to_turrets)
                    ? total_damage_dealt_to_turrets
                    : 0,
            total_damage_self_mitigated =
                int.TryParse(playerStats.TotalDamageSelfMitigated, out int total_damage_self_mitigated)
                    ? total_damage_self_mitigated
                    : 0,
            total_damage_shielded_on_teammates =
                int.TryParse(playerStats.TotalDamageShieldedOnTeammates, out int total_damage_shielded_on_teammates)
                    ? total_damage_shielded_on_teammates
                    : 0,
            total_damage_taken = int.TryParse(playerStats.TotalDamageTaken, out int total_damage_taken)
                ? total_damage_taken
                : 0,
            total_heal = int.TryParse(playerStats.TotalHeal, out int total_heal) ? total_heal : 0,
            total_heal_on_teammates = int.TryParse(playerStats.TotalHealOnTeammates, out int total_heal_on_teammates)
                ? total_heal_on_teammates
                : 0,
            total_time_crowd_control_dealt =
                int.TryParse(playerStats.TotalTimeCrowdControlDealt, out int total_time_crowd_control_dealt)
                    ? total_time_crowd_control_dealt
                    : 0,
            total_time_spent_dead = int.TryParse(playerStats.TotalTimeSpentDead, out int total_time_spent_dead)
                ? total_time_spent_dead
                : 0,
            total_units_healed = int.TryParse(playerStats.TotalUnitsHealed, out int total_units_healed)
                ? total_units_healed
                : 0,
            triple_kills = int.TryParse(playerStats.TripleKills, out int triple_kills) ? triple_kills : 0,
            true_damage_dealt_player = int.TryParse(playerStats.TrueDamageDealtPlayer, out int true_damage_dealt_player)
                ? true_damage_dealt_player
                : 0,
            true_damage_dealt_to_champions =
                int.TryParse(playerStats.TrueDamageDealtToChampions, out int true_damage_dealt_to_champions)
                    ? true_damage_dealt_to_champions
                    : 0,
            true_damage_taken = int.TryParse(playerStats.TrueDamageTaken, out int true_damage_taken)
                ? true_damage_taken
                : 0,
            turrets_killed = int.TryParse(playerStats.TurretsKilled, out int turrets_killed) ? turrets_killed : 0,
            turret_takedowns = int.TryParse(playerStats.TurretTakedowns, out int turret_takedowns)
                ? turret_takedowns
                : 0,
            unique_id = playerStats.UniqueId,
            unreal_kills = int.TryParse(playerStats.UnrealKills, out int unreal_kills) ? unreal_kills : 0,
            victory_point_total = int.TryParse(playerStats.VictoryPointTotal, out int victory_point_total)
                ? victory_point_total
                : 0,
            vision_score = int.TryParse(playerStats.VisionScore, out int vision_score) ? vision_score : 0,
            vision_wards_bought_in_game =
                int.TryParse(playerStats.VisionWardsBoughtInGame, out int vision_wards_bought_in_game)
                    ? vision_wards_bought_in_game
                    : 0,
            ...
            wards_placed_detector = int.TryParse(playerStats.WardPlacedDetector, out int wards_placed_detector)
                ? wards_placed_detector
                : 0,
            was_afk = bool.TryParse(playerStats.WasAfk, out bool was_afk) ? was_afk : false,
            was_afk_after_failed_surrender =
                bool.TryParse(playerStats.WasAfkAfterFailedSurrender, out bool was_afk_after_failed_surrender)
                    ? was_afk_after_failed_surrender
                    : false,
            ...
            enemy_missing_pings = int.TryParse(playerStats.EnemyMissingPings, out int enemy_missing_pings)
                ? enemy_missing_pings
                : 0,
            enemy_vision_pings = int.TryParse(playerStats.EnemyVisionPings, out int enemy_vision_pings)
                ? enemy_vision_pings
                : 0,
            ...
            largest_ability_damage = int.TryParse(playerStats.LargestAbilityDamage, out int largest_ability_damage)
                ? largest_ability_damage
                : 0,
            largest_attack_damage = int.TryParse(playerStats.LargestAttackDamage, out int largest_attack_damage)
                ? largest_attack_damage
                : 0,
            last_takedown_time = int.TryParse(playerStats.LastTakedownTime, out int last_takedown_time)
                ? last_takedown_time
                : 0,
            need_vision_pings = int.TryParse(playerStats.NeedVisionPings, out int need_vision_pings)
                ? need_vision_pings
                : 0,
            ...
            player_subteam_placement =
                int.TryParse(playerStats.PlayerSubteamPlacement, out int player_subteam_placement)
                    ? player_subteam_placement
                    : 0,
            puuid = playerStats.PUUID,
            rift_herald_kills = int.TryParse(playerStats.RiftHeraldKills, out int rift_herald_kills)
                ? rift_herald_kills
                : 0,
            vision_wards_cleared = int.TryParse(playerStats.VisionClearedPings, out int vision_cleared_pings)
    ...
    public static int
        intramural_season; // No default value, must be set before running the program to ensure the correct season is set
}